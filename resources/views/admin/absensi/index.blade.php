    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($kegiatans as $k)
                <a href="{{ route('admin.absensi.create', $k->id_kegiatan) }}" 
                   class="group bg-white border-2 border-gray-900 p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(22,163,74,1)] transition-all transform hover:-translate-y-1">
                    <div class="flex justify-between items-start mb-4">
                        <span class="bg-gray-100 text-gray-800 text-[10px] font-black px-2 py-1 uppercase">{{ \Carbon\Carbon::parse($k->tanggal)->translatedFormat('M Y') }}</span>
                    ...
                    <h3 class="font-black text-xl text-gray-900 leading-tight uppercase group-hover:text-green-600 transition-colors">{{ $k->judul }}</h3>
                    <p class="text-sm font-bold text-gray-500 mt-2">{{ \Carbon\Carbon::parse($k->tanggal)->translatedFormat('d F Y') }}</p>
