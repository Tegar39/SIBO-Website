...
<form method="GET" action="{{ route('admin.pendaftaran.index') }}" class="mb-8 bg-white rounded-3xl border border-slate-100 shadow-sm p-5">
            <div class="flex items-center justify-between flex-wrap gap-3 mb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Untuk menambahkan peserta manual, pilih kegiatan lalu tekan tombol Tambah Peserta.</p>
                </div>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="w-14 h-14 rounded-2xl bg-slate-100 overflow-hidden flex-shrink-0 border-2 border-white shadow-sm">
                                            @if($k->pamflet && Storage::disk('public')->exists($k->pamflet->path_file))
                                                <img src="{{ Storage::url($k->pamflet->path_file) }}" class="w-full h-full object-cover">
                                            @else
                                ...
                                <td class="px-6 py-5 align-middle">
                                    <div class="max-w-[140px] mx-auto">
                                        ...
                                        <div class="flex justify-between items-center mb-2">
                                            ...
                                            <span class="text-[10px] font-black text-slate-600 italic">{{ round($persen) }}%</span>
                                        </div>
                                        <div class="w-full bg-slate-100 h-2 rounded-full overflow-hidden border border-slate-200/50">
                                            <div class="bg-emerald-500 h-full transition-all duration-700 ease-out rounded-full" style="width: {{ $persen }}%"></div>
                                        </div>
                                ...
                                <td class="px-6 py-5 text-center align-middle">
                                    <span class="inline-flex items-center justify-center bg-emerald-50/80 text-emerald-600 font-black text-xs px-4 py-2 rounded-xl border border-emerald-100/50 shadow-sm shadow-emerald-100/20">
                                        {{ $k->pendaftarans_count }} <span class="text-[10px] opacity-70 ml-1 font-bold uppercase">Orang</span>
                                    </span>
                                </td>

                                <td class="px-8 py-5 text-right align-middle">
                                    <div class="flex flex-col sm:flex-row justify-end gap-2">
                                        <a href="{{ route('admin.pendaftaran.show', $k->id_kegiatan) }}" 
                                        ...
                                        <a href="{{ route('admin.pendaftaran.create', $k->id_kegiatan) }}" 
                                           class="inline-flex items-center justify-center gap-2 bg-emerald-600 border border-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-5 rounded-xl text-[11px] uppercase tracking-wider transition-all shadow-sm shadow-emerald-100">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="text-center py-16 bg-white/70 backdrop-blur-md rounded-[2rem] border-2 border-dashed border-slate-200/60 shadow-sm">
                ...
                <a href="{{ route('admin.pendaftaran.index') }}" class="text-emerald-600 font-bold text-xs hover:underline mt-4 inline-block tracking-widest">
