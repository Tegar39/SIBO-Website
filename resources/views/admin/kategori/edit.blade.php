...
<div class="pt-20 pb-12 bg-slate-50 min-h-screen font-sans">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white/70 backdrop-blur-md border border-white/50 rounded-[2.5rem] shadow-xl p-8 md:p-12">
            ...
            <form action="{{ route('admin.kategori.update', $kategori->id_kategori) }}" method="POST" class="space-y-8">
                @csrf 
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="deskripsi" rows="4" 
                              class="w-full bg-slate-100/50 border-none rounded-2xl p-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-emerald-500 transition-all leading-relaxed">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                </div>
